  ...
  def main(args: Array[String]): Unit = {
    ...
    while (entry != null) {
      if (!entry.isDirectory&&entry.getSize>0) {
        ...
        spark.sql("INSERT INTO acc.ods_acc_od PARTITION(partition_month) SELECT * FROM tmp_table")
        System.gc() // 手动调用垃圾回收
      }
...
/**
 * SPARK_HOME=/opt/cloudera/parcels/CDH-6.3.2-1.cdh6.3.2.p0.1605554/lib/spark  ${SPARK_HOME}/bin/spark-submit   --master yarn  --deploy-mode cluster  --driver-memory 4g --executor-memory 4g --num-executors 8 --conf spark.yarn.maxAppAttempts=1 --class com.example.hello.Targz /root/Hadoopcoding/original-spark_study-1.0-SNAPSHOT.jar
